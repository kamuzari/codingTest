...
public class KeepDistanceCheck2 {
    // P : 응시자  , O : 빈테이블 , X : 파티션
    ...
    public boolean bfs(int y,int x,char map[][],int n,int m){
        for(int i=0; i<6; i++){
            ...
            if(i<2){
                ...
                if(i==2){
                    if(map[ny-1][nx]!='X') return false;
                }else if(i==3){
                    if(map[ny][nx-1]!='X') return false;
                }else {
                    if(map[ny][x]!='X' || map[y][nx]!='X') return false;
                }
            }
        }
        return true;
